...
/*
  Originally wrote this using hooks, but have to refactor
  to be more inline with the book
...
describe('<Carousel />', () => {
  ...
  it('has an initial `slideIndex` of 0', () => {
    expect(wrapper.find('[data-index]').text()).toBe('0');
  });
  it('renders a <CarouselButton /> labeled `Prev`', () => {
    expect(
      wrapper
  ...
  it('decrements `slideIndex` when Prev is Clicked', () => {
    wrapper.find('[data-action="prev"]').simulate('click');
    expect(wrapper.find('[data-index]').text()).toBe('-1');
  });
  it('increments `slideIndex` when Next is Clicked', () => {
    wrapper.find('[data-action="next"]').simulate('click');
    expect(wrapper.find('[data-index]').text()).toBe('1');
