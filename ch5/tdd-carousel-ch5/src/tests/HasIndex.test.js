...
describe("<HasIndex />", () => {
  ...
  it("always has an `index` state equal to the `index` prop", () => {
  ...
  it("allows the `index` state to change if the `index` prop is unset", () => {
    const wrapperWithInitialIndex = shallow(
      <MockComponentWithIndex index={1} />
    );
  ...
  it("has the max `index` state on decrement of 0", () => {
